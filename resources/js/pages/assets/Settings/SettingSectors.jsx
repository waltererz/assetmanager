...
const columns = [
    { id: 'sector', label: '투자섹터', minWidth: 100 },
    {
        id: 'created_at',
        label: '생성날짜',
        minWidth: 170,
...
export default function SettingSectors()
{
    ...
    const store = async () => {
        if (!inputText || inputText.length <= 1) {
            setMessage('투자섹터 이름은 2자 이상 입력해주세요.');
            showAlertMessage();
            return;
        }

        await axios.post('/assets/settings/variables/sectors', { sector: inputText })
        .then((res) => {
    ...
    const updateData = async () => {
        await axios.get('/assets/settings/variables/sectors')
        .then((res) => {
            if (res.status == 200) {
    ...
    const remove = async () => {
        ...
        await axios.post('/assets/settings/variables/sectors/delete', { data: data })
        .then((res) => {
            if (res.status == 200 && res.data.result == 'ok') {
                updateData();
            }
                    ...
                    <Table size="small" stickyHeader>
                        ...
                        <TableBody>
                            {sectors
                                ...
                                .map((sector) => {
                                    return (
                                        <TableRow hover role="checkbox" tableIndex={-1} key={sector.id}>
                                            <TableCell padding="checkbox">
                                                <Checkbox value={sector.id} checked={checked[sector.id]} color="primary" onChange={changeChecked} />
                                            </TableCell>
                                            {columns.map((column) => {
                                                const value = sector[column.id];
                                                return (
    ...
    return (
        <React.Fragment>
            ...
            <Stack direction="row" spacing={1} alignItems="enc">
                <OutlinedInput size="small" value={inputText} name="sector_name" placeholder="추가할 투자섹터 입력" sx={{ fontSize: '0.9rem' }} onChange={changeInput} />
