...
const columns = [
    { id: 'type', label: '투자성격', minWidth: 100 },
    {
        id: 'created_at',
        label: '생성날짜',
        minWidth: 170,
...
export default function SettingInvestmentTypes()
{
    const [alertOpen, setAlertOpen] = React.useState(false);
    const [message, setMessage] = React.useState('');
    const [inputText, setInputText] = React.useState('');
    const [checked, setChecked] = React.useState({});
    const [investment_types, setInvestmentTypes] = React.useState([]);
    ...
    const store = async () => {
        if (!inputText || inputText.length <= 1) {
            setMessage('투자성격 이름은 2자 이상 입력해주세요.');
            showAlertMessage();
            return;
        }

        await axios.post('/assets/settings/variables/investment-types', { type: inputText })
        .then((res) => {
    ...
    const updateData = async () => {
        await axios.get('/assets/settings/variables/investment-types')
        .then((res) => {
            if (res.status == 200) {
                setInvestmentTypes(res.data.result);
            }
        }).catch((error) => {
            console.log(error);
        });
    ...
    const remove = async () => {
        ...
        await axios.post('/assets/settings/variables/investment-types/delete', { data: data })
        .then((res) => {
            if (res.status == 200 && res.data.result == 'ok') {
                updateData();
            }
                    ...
                    <Table size="small" stickyHeader>
                        ...
                        <TableBody>
                            {investment_types
                                ...
                                .map((investment_type) => {
                                    return (
                                        <TableRow hover role="checkbox" tableIndex={-1} key={investment_type.id}>
                                            <TableCell padding="checkbox">
                                                <Checkbox value={investment_type.id} checked={checked[investment_type.id]} color="primary" onChange={changeChecked} />
                                            </TableCell>
                                            {columns.map((column) => {
                                                const value = investment_type[column.id];
                                                return (
    ...
    return (
        <React.Fragment>
            ...
            <Stack direction="row" spacing={1} alignItems="enc">
                <OutlinedInput size="small" value={inputText} name="sector_name" placeholder="추가할 투자성격 입력" sx={{ fontSize: '0.9rem' }} onChange={changeInput} />
