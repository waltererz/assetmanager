...
const columns = [
    { id: 'country', label: '투자국가', minWidth: 100 },
    {
        id: 'created_at',
        label: '생성날짜',
        minWidth: 170,
...
export default function SettingCountries()
{
    ...
    const store = async () => {
        if (!inputText || inputText.length <= 1) {
            setMessage('투자국가 이름은 2자 이상 입력해주세요.');
            showAlertMessage();
            return;
        }

        await axios.post('/assets/settings/variables/countries', { country: inputText })
        .then((res) => {
    ...
    const updateData = async () => {
        await axios.get('/assets/settings/variables/countries')
        .then((res) => {
            if (res.status == 200) {
    ...
    const remove = async () => {
        ...
        await axios.post('/assets/settings/variables/countries/delete', { data: data })
        .then((res) => {
            if (res.status == 200 && res.data.result == 'ok') {
                updateData();
            }
                    ...
                    <Table size="small" stickyHeader>
                        ...
                        <TableBody>
                            {countries
                                ...
                                .map((country) => {
                                    return (
                                        <TableRow hover role="checkbox" tableIndex={-1} key={country.id}>
                                            <TableCell padding="checkbox">
                                                <Checkbox value={country.id} checked={checked[country.id]} color="primary" onChange={changeChecked} />
                                            </TableCell>
                                            {columns.map((column) => {
    ...
    return (
        <React.Fragment>
            ...
            <Stack direction="row" spacing={1} alignItems="enc">
                <OutlinedInput size="small" value={inputText} name="country_name" placeholder="추가할 투자국가 입력" sx={{ fontSize: '0.9rem' }} onChange={changeInput} />
